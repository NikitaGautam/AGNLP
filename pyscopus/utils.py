...
def _parse_article_science(entry):
    try:
        # scopus_id = entry['dc:identifier'].split(':')[-1]
        scopus_id = entry['pii']
    except:
...
def _parse_entry_science(entry, type_):
    if type_ == 1 or type_ == 'article':
        return _parse_article_science(entry)
    else:
        return _parse_author(entry)
...
def _parse_abstract_retrieval_science(abstract_entry):
    resp = abstract_entry['full-text-retrieval-response']
    ...
    # rename keys
    # abstract_dict['scopus-id'] = abstract_dict.pop('dc:identifier').split(':')[-1]
    # abstract_dict['scopus-id'] = resp.pop('scopus-id')
    abstract_dict['abstract'] = abstract_dict.pop('dc:description')
    abstract_dict['title'] = abstract_dict.pop('dc:title')
...
def _search_scopus(key, query, type_, view, index=0):
    ...
    par = {'apikey': key, 'query': query, 'start': index,
           'httpAccept': 'application/json', 'view': view, 'instToken': '48c77bbd9aa635051511359477cca770'}
...
def _search_scopus_science(key, query, type_, view, index=0):
    '''
        Search Scopus database using key as api key, with query.
        Search author or articles depending on type_
    ...
    par = {'apikey': key, 'query': query, 'start': index,
           'httpAccept': 'application/json', 'view': view, 'instToken': '48c77bbd9aa635051511359477cca770'}
    if type_ == 'article' or type_ == 1:
        r = requests.get(APIURI.SEARCH_SCIENCE, params=par)
    else:
        par['view'] = 'STANDARD'
        r = requests.get(APIURI.SEARCH_AUTHOR_SCIENCE, params=par)
    ...
    result_df = pd.DataFrame([_parse_entry_science(entry, type_) for entry in entries])
