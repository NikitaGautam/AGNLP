...
from pyscopus.utils import _parse_author, _parse_author_retrieval,\
        ...
        _search_scopus, _parse_serial, _parse_aff, _search_scopus_science, _parse_abstract_retrieval_science
...
class Scopus(object):
    ...
    def search(self, query, count=25, type_=1, view='COMPLETE'):
        '''
        ...
        while True:
            index = 25*i
            df1 = _search_scopus(self.apikey, query, type_, view=view, index=index)
            
            result_df = result_df._append(df1,ignore_index=True)
            if result_df.shape[0] >= count:
                return result_df[:count]
            i += 1
            
    
    def search_science(self, query, count=25, type_=1, view='COMPLETE'):
        '''
            Search for documents matching the keywords in query
            Details: http://api.elsevier.com/documentation/SCOPUSSearchAPI.wadl
            Tips: http://api.elsevier.com/documentation/search/SCOPUSSearchTips.htm
        ...
        result_df, total_count = _search_scopus_science(self.apikey, query, type_, view=view)
        print(f"Total Count Science Direct for  q = {query} =",total_count )
        ...
        while True:
            try:
                index = 25*i
                df1 = _search_scopus_science(self.apikey, query, type_, view=view, index=index)
    ...
    def retrieve_abstract(self, scopus_id, download_path=None, view='FULL'):
        ...
        par = {'apikey': self.apikey, 'httpAccept': 'application/json' , 'view': view, 'instToken': '48c77bbd9aa635051511359477cca770'}
        url = '%s/%s'%(APIURI.ABSTRACT, scopus_id)
    ...
    def retrieve_abstract_science(self, scopus_id, download_path=None, view='FULL'):
        '''
            Retrieve publication abstracts
            Details: https://api.elsevier.com/documentation/AbstractRetrievalAPI.wadl
        ...
        par = {'apikey': self.apikey, 'httpAccept': 'application/json' , 'view': view, 'instToken': '48c77bbd9aa635051511359477cca770'}
        url = '%s/%s'%(APIURI.ABSTRACT_SCIENCE, scopus_id)
        r = requests.get(url, params=par)
        ...
        try:
            return _parse_abstract_retrieval_science(js)
        except:
            raise ValueError('Abstract for %s not found!' %scopus_id)
